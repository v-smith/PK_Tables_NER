...
def convert(
        input_path: Path = typer.Option(default='../data/model_vs_annotator/reviewed_out/model_compare_review_train.jsonl'),
        output_path: Path = typer.Option(default='../data/spacy2/train_noPopDem.spacy/'),
    ...
    for annot_sentence in raw_annotations:
        ...
        cut_spans = [x for x in annot_sentence["spans"] if x["label"] != "Population" and x["label"] != "Demographics"]
        annot_sentence["spans"] = cut_spans
